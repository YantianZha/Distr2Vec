...
class GM(object):
    ...
    def train_and_test_core(self, testing_ids, testing_indices):
        ...
        for use_hs in hs_list:
            ...
            train_sentences_dup = models.uncertainWord2vec.DataLoader(self.domain, self.files, 'certain-observation', self.num_train,
                                                                      self.beam_size, training_ids, self.distr_sz)
            # Get statistics
            print "PER: ", train_sentences_dup.get_sequence_PER([2])
            model_dup = models.UncertainWord2Vec(uncertSentences=train_sentences_dup, uncertainTrain=False, min_count=1,
                                                 sg=1, workers=8, hs=use_hs, negative=0, window=self.winSz, iter=self.iter, sample=0, model=3)
            ...
            print "GM Testing : Running on data %s with H-Softmax = %d" % (self.folderName, use_hs)
            ...
            for itr, plan in enumerate(plans):
                ...
                plan_dup = list(zip(*NoisyPlans[itr])[0])
                blank_count, indices, incomplete_plan_dup = obtain_incomplete_plan(self.mode, self.missing, plan, testing_indices[testing_ids[itr]])
                total += blank_count
                ...
                best_plan_args_dup, weights_dup = EMTesting(weights_dup, self.winSz, indices, incomplete_plan_dup, blank_count,
                                                            vocab_size_dup, actions_dup, self.topk, model_dup, 'uw2v2')
