...
class RepeatUncertainCorpusNTimes(SaveLoad):
    ...
    def __iter__(self):
        for _ in xrange(self.n):
            for (sent, conf, reli) in zip(self.corpus[0], self.corpus[1], self.corpus[2]):
                # np.array(
                # yield (sent, conf, reli)
                # assert all(isinstance(x, str) for x in sent), (sent, conf, reli)
                # assert all(isinstance(x, np.float64) for x in conf), (sent, conf, reli)
                yield (sent, np.array(conf).astype(np.float64), np.array(reli).astype(np.float64))
