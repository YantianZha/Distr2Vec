...
try:
    ...
    def train_batch_sg(model, sentences, alpha, work=None):
        ...
        for sentence in sentences:
            ...
            print "word_vocabs", word_vocabs
            print "model.window", model.window
            print "word_vocabs[0]", word_vocabs[0]
            print "word_vocabs[1]", word_vocabs[1]
            print "word_vocabs[2]", word_vocabs[2]
            for w in sentence:
    ...
    def train_batch_sg_biasWin(model, sentences, alpha, work=None, winBias=-1):
        """
        Update skip-gram model by training on a sequence of sentences.
        ...
        for sentence in sentences:
            ...
            for pos, word in enumerate(word_vocabs):
                # now go over all words from the (reduced) window, predicting each one in turn
                start = max(0, pos - model.window + winBias)
                for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + winBias + 1)], start):
                    # don't train on the `word` itself
                    # print "pos2, word2", pos2, word2
                    if pos2 != pos:
                        train_sg_pair(model, model.wv.index2word[word.index], word2.index, alpha)
    ...
    def score_sentence_sg_biasWin(model, sentence, work=None, winBias=-1):
        """
        Obtain likelihood score for a single sentence in a fitted skip-gram representaion.
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            self, sentences=None, size=100, alpha=0.025, window=5, biasWin=0, min_count=5,
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        if self.sg:
            if self.biasWin == 0:
                tally += train_batch_sg(self, sentences, alpha, work) #interface
            else: tally += train_batch_sg_biasWin(self, sentences, alpha, work) 
        else:
        ...
        def worker_loop():
            ...
            while True:
                ...
                for sentence_id, sentence in job:
                    ...
                    if self.sg:
                        if self.biasWin == 0:
                            score = score_sentence_sg(self, sentence, work) # interface
...
class LineSentence_CV(object):
    """
    Simple format: one sentence = one line; words already preprocessed and separated by whitespace.
    """

    def __init__(self, source, dataIndexes, max_sentence_length=MAX_WORDS_IN_BATCH, limit=None):
        """
        `source` can be either a string or a file object. Clip the file to the first
        `limit` lines (or no clipped if limit is None, the default).
    ...
    def __iter__(self):
        ...
        #     # Things that don't have seek will trigger an exception
        #     self.source.seek(0)
        #     for line in self.islice(fin): #itertools.isfilter(self.testing_ids,self.source):
        #         line = utils.to_unicode(line).split()
        #         i = 0
        #         while i < len(line):
        #             yield line[i: i + self.max_sentence_length]
